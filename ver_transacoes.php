...
<div id='conteudo'>
    <h1> Lista de todas as transferências bancárias efetuadas</h1>

    <?php
      $sqlQuery="SELECT * FROM transferencias;";
	
      $result = $conn->query($sqlQuery);
      
      if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
          echo "<p><p>".$row['origem']." transferiu ".$row['valor'] ." € para ".$row['destino']." (". $row['descricao']." )";
        }
      }
